...
module.exports = (db) => {
    ...
    database
      ...
      .then((user) => {
        if (user) {
          ...
          database
            .updateUserTokenById(user.id, resetToken)
            .then((user) => {
              ...
              let mailOptions = {
                ...
                html: `<p>Hi, to reset your password click on the following link: <a href="${
                  req.protocol
                }://${req.get(
                  "host"
                )}/api/mailer/reset?token=${resetToken}">Reset your password</a></p>`,
              };
  ...
  router.get("/reset-password", (req, res) => {
    database
      .getUserByResetToken(req.session.reset)
      .then((user) => {
  ...
  router.post("/change-password", (req, res) => {
    ...
    database
      .updateUserPasswordByUserId(req.session.reset, cryptedPassword)
      .then((user) => {
        req.session = null;
        res.redirect("/login");
      })
